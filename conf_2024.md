...
* The ACR Mammography Positioning Improvement Collaborative: A Multicenter Improvement Program Within a Learning Network Framework `ACR Journal` `USA`
	> Improvement (Training) Program -- following Toyota A3 guidelines -- is implemeted in six sites (across four states) to help improve Mammography Positioning. The program performed current state analysis, identified root causes of problems, and determined key drivers influencing mammography positioning, proposed development and refinement of interventions. No AI models used; just a training problem that helps positioning quality.
# arXiv

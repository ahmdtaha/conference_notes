...
* SPADES: A Realistic Spacecraft Pose Estimation Dataset using Event Sensing `LU` `SnT` `ICRA`
	> The paper releases a new dataset for spacecraft pose estimation. The dataset contains both real and synthetic images. The paper proposes a mask filtering approach to filter-out low quality data (event/frames) from training dataset. The paper also propose a 3C representation to apply standard deep learning appraoches on Event-sensor data.

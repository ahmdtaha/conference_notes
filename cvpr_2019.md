...
* Iterative Residual Refinement for Joint Optical Flow and Occlusion Estimation `Optical Flow`
	> The paper estimate optical flow using U-shaped deep network. these networks have encoder-decoder architecture, and the the loss function is typically applied multiple decoder scales/levels. The paper proposes the following ideas to boost optical flow accuracy while increasing the computational efficiency.
	1. an iterative residual refinement (IRR) scheme that refines the optical flow estimation from a previous iteration. The paper shared weights between IRR networks whenever possible.
	2. The paper integrates both occlusion estimation and bi-directional flow estimation (both forward and backward) to boost performance.
	3. Integrate bilateral filtering and occlusion upsampling for refined occlusion and motion boundaries.

	
	* On a separate note, It seems like optical flow networks can only be trained either
		1. on synthetic data, in such case both forward and backward flows can be used for supervision.
		2. on real data, in which only forward flow are typicall provided because ground-truth backward flow would be difficult to compute/estimate. In real dataset, the ground-truth forward flow is computed using stereo camera and ego-car odometry and LiDAR scanner. The setup is need to map a 2D pixel into a 3D position, align points together from future frames, and finally obtain the ground-truth optical flow.

* Events-to-Video: Bringing Modern Computer Vision to Event Cameras `Pose` `Tracking` `SnT` `Event` `CH` `Academia`
	> The paper propose a fully supervised deep learning approach to convert event stream into natural video. The paper collected synthetic video frames, then produce the corresponding event data using ESIM. The authors used a UNet to predict the video frames using event data and previous video frames. The training object is a perceptual loss comparing the gt and pt frames using VGG features.

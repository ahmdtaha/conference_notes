...
* Transformers Can Do Bayesian Inference `GE` `Academia` `Bosch`
	> Motivation: It is either computationally expensive or very complex to model posterior distribution using deep learning models. Accordingly, there is no simple way to compute the posterior predictive distribution PPD P(y_test|x_test,D_train)
	
	> Proposal: The paper proposes a Prior fitted-network (PFN) that learns the distribution prior using synthetic data, and embeds that prior into a transformer model during training. During inference, PFN is  presented with both the real training samples D_train and the query sample x_test to generate the PPD. During inference, the real training samples serve as in-context learning (ICL), guiding the PFN (transformer) on how to make a prediction for x_test. To inject a prior into PFN, the authors sample synthetic samples from an artificial (Fake) Bayesian Neural Network BNN.
	
	> Results: PFNs can perfectly mimic Gaussian processes and demonstrated promising results on Tabular data.

* Open-vocabulary Object Detection via Vision and Language Knowledge Distillation `Google` `Nvidia`
	> Motivation: How to develop an open-vocab detector without the costly data curation process.
	
	> Method: The paper proposes ViLD, an open-vocabulary detector that replaces the standard classification layer with a vision embedding layer matched with text embedding. The classification embedding head is trained to maximize the cosine similarity with the text embedding for labeled data. To promote generalization to novel, unlabeled classes, the paper utilizes a distillation loss between the classification embedding output and a pretrained VL model (e.g., CLIP or ALIGN).

* Not All Patches are What You Need: Expediting Vision Transformers via Token Reorganizations `HK` `Tencent`
	> Motivation: ViT scales well with data and compute but is expensive during inference due to the large number of tokens. If irrelevant tokens can be dropped or merged, that would reduce cost without undermining accuracy.
	
	> Method: The paper uses the attention weights between [CLS] token and patch tokens to identify attentive and inattentive tokens. Attentive tokens are preserved while inattentive tokens are fused. The papers found that applying this approach directly during inference leads to inferior performance. Accordingly, the proposed ViT, EViT, is trained to fuse inattentive tokens. This way there is no shift between the training and inference behavior.	
...
* Grounded Language-Image Pre-training `VLP` `MSF`
	> The paper re-propose CLIP but with grounding (bbox detection) capabilities, i.e., GLIP. GLIP casts object detection as a grounding task without context (list all possible class labels). Accordingly, GLIP tackles both vanilla object detection and grounding tasks. While CLIP uses late-fusion (dot-product), GLIP uses intermediate deep-fusion (cross-attention) to enhance the text and image (bounding box) features. Besides the proposed model, the paper highlights the importance of grounding-data to train a superior teacher model. A superior teacher model generates better and more diverse pseudo bbox for training the student model. Concretely, the teacher model is trained on 0.8M gold grounding data, then used to generate pseudo annotations for 24M web-crawled image-text pairs.
	
* Robust Contrastive Learning against Noisy Views `US` `MIT` `Microsoft`
	> The paper proposes a symmetric NCE loss called RINCE. By having a symmetric NCE, contrastive learning becomes more robust to noisy positive views. The proposed loss, RINCE, provides a lower bound on wasserstein dependency measure (WDM) -- a metric for mutual information (MI). While vanilla MI uses KL divergence, WDM uses Wasserstein distance to measure the distance between the joint and marginal distributions. Unlike KL, Wasserstein distance is more robust to noisy views and better equipped to capture the geometry of the representation space. Contrastive learning maximizes MI by separating positive pairs ( joint distribution) away from negative pairs (marginal distributions). Such maximization manifest into a large margin between different clusters, i.e., a large inter-class distance. The proposed RINCE Loss has a hyperparameter q \in [0,1] such that q=0 becomes a non-robust NCE, while q=1 becomes a fully robust NCE loss. The key idea behind RINCE loss is to focus (assign higher weight) on easy positives and less weight to hard (potentially noisy) positives. In an over-simplistic view, RINCE is like the opposite of focal loss. While the q hyperparameter controls robustness, it introduces a trade-off between robustness and learning (convergence) speed. By focusing more on easy positives and less on hard positives, RINCE converges more slowly compared to NCE in a clean setup. While RINCE treats (assigns weights to) different positive samples differently, all negative samples are assigned the same weight -- all negatives are treated equally.
...
* Next-ViT: Next Generation Vision Transformer for Efficient Deployment in Realistic Industrial Scenarios `ByteDance` `Nice`
	> The following are responsible for the inefficient vanilla ViT models: quadratic complexity of attention, non-foldable LayerNorm, and GELU layers. Hybrid architecture runs efficiently on TensorRT and CoreML. Propose a new hybrid architecture with efficient multi-head convolution attention to model short-range dependencies and vanilla attention for long-range dependencies. Ideas#1: Long-range modeling is important for multi-stage architectures used in detection and segmentation tasks. Ideas#2: A single long-range attention layer is enough per stage after performing short-range (convolutional) attention. Idea#3: We can control (reduce) the number of channels in before attention using DWConv.
...
* MAR: Masked autoencoders for efficient action recognition `Alibaba` `Singapore`
	> Speedup ViT training on videos by dropping a percentage of tokens (patches/tubelets). Given the high redundancy of the spatial-temporal signal, no need to process all tokens. Use a bridging classifier to reduce the gap between low-level reconstruction features and high-level semantic features. 

* Language Models are General-Purpose Interfaces `Microsoft` `US` `Nice`
	> The paper presents one prespective on how bi-directional and causal encoders can be regarded. The paper starts by stating few key observations:
	1. Causal language modeling (i.e., unidirectional decoder) is conducive to zero-shot generaliza-
tion and in-context learning
	2. Non-causal modeling (i.e., bidirectional encoder) is conducive to transfer across tasks, lan-
guages, and modalities
	3. Semi-causal language modeling (combining uni/bi-directional encoders) as a meta-pretraining task.
	4. Non-causal encoders as System 1, and causal language models as System 2.
	These observations explains why we fine-tune LLMs and freeze multi-modal encoders. The paper describes the causal LM as a universal task layer (i.e., general-purpose interface) that can operates on any aligned-modalities and unifies various tasks as free-text generation.
	The paper presents a new figure (no. 3) which summarize different encoding approaches: (1) Causal LM, a.k.a. decoder arch, (2) Prefix LM, a.k.a encoder-decoder arch, (3) non-causal/bidirectional LM, a.k.a encoder arch, (4) semi-causal LM, a.k.a large multi-modal language models.
	
* OPT: Open Pre-trained Transformer Language Models `Meta` `US`
	> Meta release open-source foundation models for the research community. The paper hopes these model will help develop clear guidelines around responsible AI -- studying bias and toxicity. 
	
* Investigating Power laws in Deep Representation Learning `Mila` `CA`
	> The paper propose a way to evalaute the quality of embedding. The method starts be computing the eigenspectrum of co-variance matrix. Then, find the slope \alpha of a power law (\lambda_i ~ i^{-\alpha) that fits the eigenspectrum. A \alpha~=1 has been found to indicate better generalization. One thing I learned from this paper is that `Covariate` is simply a statistical term for input variable.
...
* ViTPose: Simple Vision Transformer Baselines for Human Pose Estimation `Pose` `ViT` `CN` `Academia` `AU` `NIPS`
	> The paper proposes an efficient ViT baseline for Pose estimation. The architecture is promoted as efficient, flexible, scalable, and simple. The paper indeed kept the architecture simple by using a simple decoder operating on the last feature map, so there is no feature pyramid network. The only trick -- for performance -- was the attention formulation. The paper changes the naive attention operation into window-based attention (dynamic windows and/or pooling).  
	
* Joint Embedding Predictive Architectures Focus on Slow Features `NIPS` `Workshop` `NYU` `Meta` `CIFAR` `US`
	> The paper evaluates JEPA against reconstruction pre-training in a toy setup. The toy setup involves a state(image)-action sequences. A ball is moved on a 2D image, and the action corresponds to the ball's displacement. The key conclusion is that JEPA has the potential to avoid model collapse with the right regularizer (e.g., VICReg). The paper also shows a possible failure case for JEPA in the presence of slow-features, i.e., fixed noise.
...
* BLIP: Bootstrapping Language-Image Pre-training for Unified Vision-Language Understanding and Generation `Salesforce` `US` `ICML`
	> The paper propose a VLM to support both retrieval and understanding-based tasks
or generation-based tasks. The proposed VLM delivers (1) unimodal image/text encoders, (2) image-grounded text encoder, (3) image-grounded text decoder. To train these components, the VLM leverages a suite of objectives: (1) contrastive learning, (2) an image-text matching (ITM) binary cross-entropy loss matching image-text pairs, (3) auto-regressive loss to generate image captions. Besides the propose VLM, the paper found that high-quality image-text pairs are important to achieve superior  performance. Accordingly, the paper propose CapFilt that contains a two modules: (1) a captioner fine-tuned using auto-regressive loss, (2) a filter module fine-tuned using the ITM loss objective on high quality image-text pair dataset such as COCO dataset.

* GLIPv2: Unifying Localization and Vision-Language Understanding `Microsoft` `US` `NIPS`
	> Motivation: Build a single VLM model capable of both localization, and VL understanding tasks (captioning & VAQ).
	
	> Method: Train a VLM architecture with three components: vision encoder, text encoder, and fusion encoder. This differs from 2025 VLM models in at least two aspects: (1) the vision encoder generates a fine-grained dense grid of spatial tokens compared to a single global token, (2) the fusion encoder uses bi-directional attention by default for both text and image tokens, compared to LLM decoder models in recent LLMs. The paper uses bi-directional attention during pre-training, but also states that I can switch to causal attention when fine-tuning for caption generation (language modeling).
	
	> Results: The proposed architecture, GLIPv2, is capable of both localization, grounding tasks as well as VL understanding tasks (VQA and captioning). 
	
* Stop Measuring Calibration When Humans Disagree `NL` `DK` `DE` `Academia` `WR` `JRC`
	> Motivation: We need better metrics than ECE for problems with high human disagreement.
	
	> Method: The paper proposes three instance-level calibration metrics: Human Entropy Calibration Error, Human Ranking Calibration Score, and Human Distribution Calibration Error. The paper shows that these metrics deliver meaningful measures for the Oracle classifier, while the standard ECE struggles. Furthermore, the paper highlights also the limitations of temperature scaling in high-disagreement problems involving humans.
	
* SC6D: Symmetry-agnostic and Correspondence-free 6D Object Pose Estimation `3DV` `FI`
	> The paper proposes a symmetry-agnostic and correspondence-free 6D pose estimation approach. Correspondence-free means the approach doesn't predict keypoints to estimate pose -- e.g., using PnP -- but regresses the pose (orientation and translation) directly. The paper focuses on symmetric (textureless) objects. Given that multiple orientation as plausible for a given object, the paper casts the problem as a contrastive learning approach, where an object representation embedding is matched (contrasted) with orientation embeddings. The orientation embeddings are learned using a light-weight model taking SO(3) (9D) vectors and generating an orientation embedding. The paper made an interesting decision choice to learn allo-centric orientation and not ego-centric orientation. Two objects with the same orientation would have the same allo-centric orientation -- but different ego-centric orientation -- even if they appear in different image crops. By using allo-centric orientation, it becomes possible to formulate the pose-estimation problem as a constrative learning problem.
	
* Cross-domain Contrastive Learning for Unsupervised Domain Adaptation `CN` `Academia` `IEEE` `MULTIMEDIA`
	> The paper proposes learning pseudo-labels on the fly for domain adaptation in classification problems. There are two key ideas: (1) When source data is available, use it to create class centroids as initialization for k-means, then cluster target samples around these centroids to acquire pseudo-labels; (2) When source data is not available, use class weight vectors -- from the classifier layer -- as initialization for k-means and repeat the same process to assign pseudo-labels to target samples. In this scenario, only the feature extractor weights are updated while the classifier layer is frozen. The proposal supports classification problems only. While it is advantageous to learn these pseudo-labels on the fly during training, i.e., a single stage training process, it is expensive to perform K-means for the entire target dataset during training.
...
* Transformers Can Do Bayesian Inference `GE` `Academia` `Bosch`
	...
	> Proposal: The paper propose Prior fitted-network (PFN) that learns the distribution prior using synthetic data, embed that prior into a transformer model during training. During inference, PFN is  presented with both the real training samples D_train and the query sample x_test to generate the PPD. During inference, the real training samples serve as in-context learning (ICL) purpose guiding the PFN (transformer) into how to make a prediction for x_test. To inject a prior into PFN, the authors samples synthetic samples from an artificial (Fake) Bayesian Neural Network BNN.

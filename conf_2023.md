...
* MiniGPT-4: Enhancing Vision-Language Understanding with Advanced Large Language Models
	> With frozen large language/vision models, we can build a large multimodal model by learning a single linear-projection layer; projecting the image encoding to the right embedding space.
* ConvNets Match Vision Transformers at Scale `DeepMind` `Google`
	> NFNets matches ViTs when trained with comparable compute budgets. Language and High resolution image evaluations are omitted!
...
* Mask DINO: Towards A Unified Transformer-based Framework for Object Detection and Segmentation
	> Universal architecture for both detection and segmentation

...
* Impact of Artificial Intelligence–driven Quality Improvement Software on Mammography Technical Repeat and Recall Rates `Rad AI` `WR`
	> The paper used AI model, not to give real-time feedback to Techs and Rads, but to give feedback within regular feedback sessions. Techs are able to view their Image Quality scores and identify areas of improvements. The paper focused on Position-ing and compression problems only and not blur. Quantitative evaluation is performed between `baseline` period and `current` period, i.e., no direct comparison between AI and Rad.  

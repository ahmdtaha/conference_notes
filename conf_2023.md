...
* Overcoming Simplicity Bias in Deep Networks using a Feature Sieve `ICML` `Google` `IN`
	> The paper identifies spurious (simple) features an auxiliary classiers on top of early layers. Once identified the paper uses "adversarial" loss to earse /forget these features. Accordingly, the network gets to focus on complex -- real -- features.
...
* RankMe: Assessing the Downstream Performance of Pretrained Self-Supervised Representations by Their Rank `US` `Meta` `FR`
	> The paper propose matrix rank as a metric to evaluate embedding (represetation) spaces learning by different variants of the same SSL method (e.g., DINO). A High rank corresponds with high performance in down-stream tasks -- even on out-of-distribution tasks.
...
* Semi-Supervised Contrastive Learning for Deep Regression with Ordinal Rankings from Spectral Seriation `NIPS` `HK` `CN` `Academia`
	> The paper applies spectral seriation on top of a feature similarity matrix to acquire pseudo-ranking for unlabeled samples. The pseudo-ranking is used as a supervision signal for semi-supervised learning—for unlabeled sampling. The paper applies its proposal to regression (ranking) tasks, e.g., estimating person age from an image. 

* LIMA: Less Is More for Alignment `NIPS` `Meta` `CMU` `US`
	> Motivation: Do we need an expensive instruction-alignment fine-tuning phase after pre-training?
	
	> Findings: The paper shows that a small high quality and diverse instruction-alignment dataset (1k samples) is enough to deliver an LLM competitive with state-of-the-art LLM. This result indicates the following: (1) the pre-training stage is the core stage doing most of the heavy lifting, while the instruction fine-tuning stage can be alleviated by a small high quality sample. (2) The paper also shows diminishing returns when scaling the fine-tuning dataset if quality and diversity are not taken into consideration. That said, it is important to highlight the paper's approach limitation: (1) this approach is not scalable: One needs to carefully handle every corner case (e.g,. multi-turn prompts, toxic and adversarial prompts,  etc); (2) preparing high quality and diverse prompt is a highly intellectual process -- it requires field-experts making it very expensive even when constructing a small 1K-sample dataset.
...
* Lost in the Middle: How Language Models Use Long Contexts `TACL` `US` `Stanford`
	> Motivation: Evaluate if LLMs with large context window is superior to baseline models.
	
	> Findings: Both large and small context window suffer U-shaped performance curves, where performance is superior when relevant informations appears at the beginning or end of the context. The paper highlights few approaches to mitigate performance drop: (1) Encoder-decoder LLMs are more robust to the position of relevant information; (2) Adding the query to both the beginning and end of the prompt helped with synthetic key-value retrieval task, but minimal help to QA task; (3) Neither pre-training nor instruction fine-tuned models help mitigate the U-shaped performance curve. In sum, the paper concluded that long-context presents a trade-off: it increases the chances of containing the correct answer leading to higher performance, but also increases the reasoning burden on LLMs leading to lower performance.
	
* In silico environmental sampling of emerging fungal pathogens via big data analysis `Fungal Ecology` `AU` `Academia`
	> Problem: There is an emergence of multi-drug resistent fungi species. This needs attention to identify sources and drivers for such emergence. E.g., is this due to climate change.
	
	> Finding: This is a computational analysis study, where sample datasets from Sequence Read Archive (SRA) are analyzed to identify any patterns (e.g., geographic region, climate conditions) contributing to the emergence of these multi-drug resistant species. The paper's analysis finds that this class of fungi ( Candida haemulonii) spans a large range of geographic regions (Antarctica, the Arctic, and the Pacific Ocean, china, USA), hosts (human, plants, insects, amphibians), and climate conditions (tropical, arid, temperate, continental, and polar climates). Other findings of the paper is that the ITS region -- in Candida species -- can reliably separate species of interest. The emergence of this multi-drug resistant species are hypothesized to be due to expansion industrial farming and antifungals usage.
	
	> Things I learned: The polymerase chain reaction (PCR) is a technology -- awarded Noble prize -- help amplify (replicate/photo-copy) certain parts of the DNA sequence to enable higher accuracy DNA read. Dataset samples are imbalance, dominated by the host sample (Human/Soil) which makes it challenging to separate the DNA sequence of interest (e.g., Candida DNA sequence).

* Attention-Guided Transfer Learning for Identi cation of Filamentous Fungi Encountered in the Clinical Laboratory `Microbiology Spectrum` `TW` `CN` `Academia`
	> The papers uses CNNs (e.g, MobileNet, Xception) -- with a custom attention component -- to identify Filamentous Fungi. The paper curated a dataset of 4108 microscopy images that are downsampled into 224 during training. The paper released the collected dataset on GoogleDrive. 
...
* Object Pose Estimation with Statistical Guarantees: Conformal Keypoint Detection and Geometric Uncertainty Propagation `US` `Nvidia` 
	> The paper uses conformal prediction for uncertainty estimation. The paper estimates keypoint uncertainty (confidence) using a calibration dataset. Yet, the key paper's contribution is moving from keypoint uncertainty to pose uncertainty. This is not trivial because the set of all possible poses is non-convex. To solve this non-convex problem, the paper relaxes the problem into a semidefinite-positive problem. The paper uses MOSEK optimization to solve the SDP. 
	
	
* DARE-GRAM : Unsupervised Domain Adaptation Regression by Aligning Inverse Gram Matrices
	> The paper proposes a method (DARE-GRAM) to reduce the domain gap in regression problems. The key idea is to focus on the Inverse Gram matrix in the ordinary least squares OLS formulation: w = (Z^TZ)^{-1}Z^T Y, where w is the linear regression layer weights, Z denotes the input features, and Y denotes the regression targets (labels). The paper explains that even if the features (Z) are aligned, a domain gap can arise from divergent Inverse Gram matrices between source and target domains. Accordingly, the paper aligns the Inverse Gram between source and target domains. The paper aligns both the angle and scale of the Inverse Gram. The scale alignment is achieved by aligning the eigenvalues of the Gram matrix, which is equivalent to the Inverse Gram. The angle alignment is performed directly on the Inverse Gram. While RSD (a previous approach) aligns the singular vectors of Z, this paper (DARE-GRAM) aligns the columns of the Inverse Gram using cosine similarity. The paper elaborates on why this is a better approach than the singular/eigenvector approach.
	
* MaPLe: Multi-modal Prompt Learning `AE` `AU` `SE` `Academia`
	> The paper proposes multi-modal prompt tuning for CLIP-based models. Instead of tuning text-only prompt tokens, the paper tunes both text and vision prompt tokens. The paper conditions the vision token on the text token to ensure "mutual synergy". Furthermore, the paper tunes vision/text tokens for each layer -- up to J layers -- instead of tuning tokens for the first layer only. 
...
* Self-regulating Prompts: Foundational Model Adaptation without Forgetting `Google` `AE` `AU` `SE` `US`
	> The paper proposes three regularization techniques for multi-modal prompt tuning: (1) aligning the prompted features with the pre-trained frozen (CLIP) features, i.e., before fine-tuning starts, (2) combining (ensembling) prompt tokens using weighted averaging to reduce overfitting, and (3) generating and using diverse text captions during fine-tuning to mitigate prompt overfitting to a small set of text captions -- one prompt per class category.
	
* SpyroPose: SE(3) Pyramids for Object Pose Distribution Estimation `Workshop` `DK` `Academia` `Pose`
	> The paper proposes a pose-distribution estimation model to handle symmetric, textureless objects. The pose distribution is represented as a tree with the top level denoting coarse low-resolution predictions while the bottom level denoting fine high-resolution predictions. During training, the proposal contrasts the predicted pose across all poses in the top level, then the top-k poses are selected to contrast the predicted pose with a subset of poses (top-k's children) in lower levels. The tree representation delivers multiple benefits: (1) It allows importance sampling during training, which is essential to train a model with informative negatives, (2) It provides a likelihood for the true pose or any possible pose to test. 
...
* Token Merging: Your ViT But Faster `Meta` `GT` `US` `Oral`
	> Motivation: Huge ViT achieve superior performance, but expensive to run during inference. ViT Pruning approaches reduce inference cost, but either require re-training, limit token reduction or impractical for batched inference.
	
	> Method: Instead of pruning, the paper proposes a token merging (ToMe) technique. Based on the cosine similarity between token, pairs of tokens are merge to reduce the total number of tokens by r (quantity, not percentage). The cosine similarity is computed using the tokens' Key embedding -- from the attention operation. To efficiently identify the top r similar tokens to merge, the paper uses bipartite soft matching. The bipartite approach because it is efficient (non-iterative), simple and computationally cheap.
	
* Linearly mapping from image to text space `Brown` `US` `Academia`
	> The paper shows that visual encoders' embedding to similar to text encoders' embeddings by up to a linear transformation. The paper shows that by feeding visual embedding into a text decoder captioner. The visual encoder and text decoder are connected using a tunable linear projection that is fine-tuned to generate captions. The trained linear projection layer achieved good performance on captioning and VQA tasks. This indicates a high similarity between the visual and textual embedding spaces. That said, not all visual encoders learn an equally good embedding space. The paper shows that visual encoders trained with linguistic supervision (e.g., CLIP) outperform those trained with less linguistic supervision (e.g., masked autoencoders).

...
* LLaVA-Med: Training a Large Language-and-Vision Assistant for Biomedicine in One Day `Microsoft` `NIPS`	
	> LLaMA => LLaVA => LLaVA-Med. The paper (1) extracts image-text (caption) pairs from PubMed dataset, (2) create instruction-following dataset automatically using  GPT-4, (3) Fine-tune a generalized VLM model (LLaVA) into Biomedical domain using two-stages pipeline. The first stage align vision and text embedding using auto-regressive task: Given an image and an instruction to explain the image, the model generates the image caption. The second stage train the model on instruction-following data to master open-ended conversations.
	
* BLIP-2: bootstrapping language-image pretraining with frozen image encoders and large language models `Salesforce` `US` `ICML`
	> The paper propose a two-stage pipeline to train a VLM model on top of frozen vision and LLMs. Thus, The key contribution is computational efficiency during training. To acheive such objective, the paper propose a Q-former (query-former); this components acts like a linear-projection between the visual and language representations. Yet, it has more responsibility (functionality) since the LLM is completely frozen in BLIP-2, a constraint that has been relaxed in followup VLM papers. Yet, the core idea here is that BLIP trains a Q-former only while keeping the vision and language model frozne. The Q-former is trained in two-stages: (1) this first stage bootstrap vision-language representation Learning using three objective previously proposed in BLIP, i.e., ITC, ITM, LM; then, (2) the second stage bootstrap vision-to-language generative learning.

...
* The Devil is in the Details: Delving into Unbiased Data Processing for Human Pose Estimation `CN` `Academia` `SnT` `Pose`
	> The paper identifies two sources of bias in common pose estimation pipelines. The paper tackles this bias by proposing unbiased coordinate-system transformations and unbiased keypoint-format transformations. The first transformation (coordinate) describes an unbiased method to map (transform) keypoint coordinates from source to image to output coordinates without introducing irreducible error. It is worth noting that the coordinate system bias only manifests when applying geometric transformation (e.g., flipping) only. The other transformation (keypoint) describes how to encode and decode keypoint predictions such that no errors are introduced by the pre- and post-processing pipelines, i.e., any error comes from the network.
...
* RAFT: Recurrent All-Pairs Field Transforms for Optical Flow `US` `Princeton`
	> The paper proposes an optical flow estimation approach that mimic classical optical flow approaches but using deep learning components. The proposed approach (RAFT) has three components:
	1. Pixel-level feature extract in parallel with a context encoder
	2. A correlation layer that construct 4D correlation pyramid (volume). 
	3. A recurrent neural networks that update flow estimate using the correlation pyramid and context features.
	The  correlation layer mimics the data term in classical approaches, while the paper introduce some technicalities to impose smoothness.
	
* UNITER: UNiversal Image-TExt Representation Learning `Microsoft Dynamics` `US`
	> The paper learns a universal V+L embedding space to support various downstream tasks: VQA, visual entailment, image-text retrieval, etc. To achieve this goal, the paper propose a single architecture that fuses both V+L modalities. On top of that, the architecture is pre-trained using four tasks: (1) masked language modeling, (2) masked region (vision) modeling, (3) Image-text alignment, (4) word-region alignment by computing optimal transport distance. 
	
* PolarMask: Single Shot Instance Segmentation with Polar Representation `HK` `AU` 
	> Motivation: Traditional segmentation approach applies a binary classification loss for every pixel (foreground/background) which is computationally expensive. A new approach is required with simpler and cheaper computational complexity.
	
	> Method: The paper formulates segmentation masks using polar coordinates -- a center point and n contour points. Accordingly, the proposed method operates on coarse-grained features (cheaper complexity) and make a fixed number of predictions independent of the mask size. Given a groundtruth instance mask, the paper explains how to pick groundtruth center point(s), centerness metric, and n contour points for training. Interestingly, the paper shows how bounding box prediction can be regarded as a special case of the proposed polar-representation instance segmentation.
...
* Test-Time Training with Self-Supervision for Generalization under Distribution Shifts `ICML`
	> 1. The paper tackles the domain-shift problem between source and target domains. The paper introduces test-time training, where the model is fine-tuned on test samples before running inference. The model is tuned on test samples using an auxiliary self-supervised task (e.g., prediction rotation). To achieve best results, the main task (e.g., object recognition) and auxiliary task gradients should align. This is not a trivial assumption or requirement. 
...
* Debiased Contrastive Learning `NIPS`
	> The paper proposes a modified, unbiased contrastive learning (NCE) function. The paper claims the original NCE loss is biased because negative samples are randomly sampled from the entire population, not the class-negative population. Accordingly, a positive dog image can be paired with another dog image as a negative image. This is likely to happen if the (pre)training dataset contains a small number of classes (e.g., MNIST, STL10). Of course, this poorly sampled negative image can undermine pre-training. Accordingly, the paper modified NCE loss to account for possible incorrect negative pairs. The proposed approach is simple: the paper assumes the positive class has probability `tau_pos` in the dataset. Accordingly, `tau_pos` \times expected similarity score (e^{f \times f^T) is subtracted from the NCE denominator. If `tau_pos` is unknown, the paper treats it as a hyperparameter.
	
	
* Universal Domain Adaptation through Self-Supervision `NIPS` `Boston` `IBM` `Watson` `US`
	> The paper tackles universal domain adaptation where the target domain could be either (1) a closed-set, (2) an open-set, or (3) a partial set of the source domain. In such a universal setting, it is possible to have target samples unknown to the source domain, i.e., new classes. Thus, traditional domain adaptation methods -- through feature alignment --  could lead to catastrophic misalignment. Instead of batch-level alignment, the paper proposes instance- or sample-level alignment via an entropy minimization objective. Basically, the proposal pulls each sample individually to its closest neighbor in the embedding space through an entropy minimization objective. In parallel, the proposal pushes the classification probabilities (the model's output) towards either high confidence for presumably "known" samples and low confidence for presumably "unknown" samples. 
...
* How does image quality affect radiologists’ perceived ability for image interpretation and lesion detection in digital mammography? `European Radiology` `Academia` `NL` `SE`
	> The paper shows that both acquisition-related issues (lower spatial resolution and increased quantum noise) and post-processing-related issues (lower and higher contrast and
increased correlated noise) impact image quality. In addition, the paper shows that different image quality issues impact lesion types differently: calcifications and soft tissues (masses, asymmetries, and architectural distortions). Concretely, Lower spatial resolution and increased quantum noise affected calcification interpretation more than soft tissue lesion or normal cases. This concludes that calcifications and soft tissue are affected differently by image quality, which challenges the assumption that “the visibility of normal anatomy is strongly correlated to the detectability of pathological structures”.

* Deep learning approach to describe and classify fungi microscopic images `PLOS One` `PL` `Academia`
	> The paper releases a small dataset (180 images) of high resolution 3600x5760x3 microscopy images for fungi classification. The paper evaluated deep learning approaches -- with linear probing -- against classic ML approaches (e.g., BOW and FisherVector). The paper finds the ML approaches delivers superior performance. The inferior DL performance is attributed to the small dataset size and DL overfitting capabilities. The dataset name is Digital Images of Fungus Species database (DIFaS)
	
* Deep High-Resolution Representation Learning for Visual Recognition `TPAMI` `CH` `Microsoft` `Research`
	> The paper proposes a new architecture called HighResolution Network (HRNet). HRNet maintains a high-resolution feature map across all stages. Besides the high-resolution stream, HRNet has small-resolution streams to encode higher semantics. HRNet fuses information across different resolution streams using (group) convolution layers -- along with downsampling and bilinear upsampling. HRNet targets computer vision tasks with high-precision requirements, such as pose estimation and semantic segmentation.
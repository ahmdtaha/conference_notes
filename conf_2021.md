...
* FILIP: Fine-grained Interactive Language-Image Pre-Training `Huawei` `CN`
	> The paper proposes a fine-grained CLIP model, FILIP. Instead of computing global similarity between image and text features, FILIP computes pairwise similarity between image and text tokens. Each image (text) token is assigned to the text (image) token with the highest similarity. Once token-level similarity and assignment are computed, FILIP averages (combines) these similarities to compute a global similarity score between the image and the text.  The global similarity scores are used for training the image and text encoders using web-crawled image-text pairs. CLIP is already computationally expensive due to the large batch requirement and the communication cost between nodes during training. With its token-level similarity computation, FILIP becomes even more computationally expensive. Accordingly, the paper uses some tricks to reduce the training cost: (1) use fp16 instead of fp32 for some layers, (2) reduce embedding size d=256, (3) FILIP keeps 25% tokens only with the highest similarity scores. These are considered the most representative tokens — the ones that matter most for cross-modal alignment. I didn't find an official implementation for FILIP, but there is an unofficial implementation by [lucidrains](https://github.com/lucidrains/x-clip)
	
### Generic 2021

* `Multimodal Neurons in Artificial Neural Networks`
	> There are neurons in vision models (e.g., ViT) that align and fire for certain concepts across multiple modalities. E.g., these neurons would fire for both a photo of Obama's face and the word "Obama". This observation aligns with similar neuron behaviour in the human brain.
...
* Implicit-PDF: Non-Parametric Representation of Probability Distributions on the Rotation Manifold `ICML` `Google` `Pose` `Symmetric`
	> The paper uses ResNet features along with MLP to learn an implicit pose distribution per image. The learned distribution aims to highlight ambiguity for symmetrical objects. I am surprised that resnet features worked out of the box without any task-tuning. The paper also proposed to visualize SO(3) space on 2D sphere.
...
* Representation Subspace Distance for Domain Adaptation Regression `ICML` `DAR` `CN` `Academia`
	> The paper tackles the problem of domain adaptation for regression problems. The paper proposes a method to align the source and target domains' features by minimizing the principal angles. Principal angles are the angles between principal vectors, which are weighted sums of orthogonal basis vectors. The proposed objective, representation subspace distance (RSD), minimize the principle angles -- the are angles between the closest pair of principle vectors from source and target domains. While RSD minimize the sum of principle angles, RSD is oblvious to the order (importance) of orthogonal bases in source and target domains -- it can mistakenly align an important bases (vector) from the source domain with an unimportant bases from the target domains. This would be unreasonable because different bases have different semantic meaning, and shouldn't be aligned together. To tackle this problem, the paper proposes another regularization term, Bases mismatch penalization (BMP), to make sure bases are aligned between source and target domains.
	
* Self-training Improves Pre-training for Natural Language Understanding `NAACL`
...
* A Data Set and Deep Learning Algorithm for the Detection of Masses and Architectural Distortions in Digital Breast Tomosynthesis `NLM`
...
* Long-read sequencing in fungal identification `Microbiology Australia`
	> This is a review paper outlining new technology for long-read sequencing. The paper presents two platforms (Pacific Biosciences and Oxford Nanopore Technologies) for long-read sequencing that support two sequencing approaches: Metabarcoding, metagenomic. Metabarcoding typically used for targeted reads; it depends on amplification technologies (PCR) and achieves high accuracy. In contrast, metagenomics typically used for global (long) reads; it breaks a long sequence into a short one -- using a "shotgun" -- then read the short fragments and computationally stitch them. To my knowledge, Pacific Biosciences and Oxford Nanopore Technologies supports both technologies but these platforms operates differently. Pacific Biosciences read sequences by attaching a polymerace to sequence and use light (fluorescent) signal to sequence (identify) nucleotides. In contrast, Oxford Nanopore Technologies uses nanopores to pull the DNA sequence, then electric current changes to to sequence (identify) nucleotides. Both platforms have promising results in fungal identifications. Yet, limitations still exists: DNA sampels are overwhelmed by irrelevant (human) DNA, no complete dataset for fungi identification so even a perfect sequence doesn't guarantee identification, bioinformatic requirements (higher sequencing accuracy, Host DNA removal, lack of fungi genome coverage in datasets).
	
* Knowledge distillation methods for efficient unsupervised adaptation across multiple domains `Image and Vision Computing` `CA` `academia`
	> 1. The paper combines both knowledge distillation (KD) and unsupervised domain adaptation (UDA) approaches to learn compact, efficient CNNs that generalize on target domain(s). KD and UDA are applied simultaneously to jointly train the teacher and student models. The KD is applied to both logits and intermediate features. The paper explores two UDA methods: (1) MMD measuring the distance between mean features inside an embedding space (Reproducing Kernel Hilbert Space with Gaussian Kernel); (2) a domain classifier trained with a gradient reversal layer.
...
* Differentiable Event Stream Simulator for Non-Rigid 3D Tracking `Pose` `Tracking` `Workshop` `Google` `DE`
	> The paper proposes a differentiable event simulator. Given a pair of object-model (e.g., hand object) params (current and previous step), the simulator renders both image frames, then subtracts them to generate (render) event data. The current object-model is a candidate pose that is rendered and compared with observed event-camera output. The different between the observed and rendered event data is a loss that guides the candidate pose optimization towards a better pose. The proposed simulator assumes event-modality only (no RGB or depth modalities are required). The proposed method supports non-rigid objects with either a morphable  model or a 3D mesh representation. Given the optimization nature of this approach, it is Not real-time; it takes 1.5 minutes per frame on a V100 GPU.
	
### ICCV 2021

* Is it Time to Replace CNNs with Transformers for Medical Images? `Workshop`

* The Many Faces of Robustness: A Critical Analysis of Out-of-Distribution Generalization `Google` `US` `UC Berkely`
	> The paper collects four new datasets to study robustness across different directions: (1) camera position, (2) style -- drawing, (3) blur, (4) geographic location. Furthermore, the paper proposes a new augmentation technique where training images are fed into an image-to-image (e.g., autoencoder) architecture and perturbations are applied to the weights/features of the architecture to reconstruct a noisy/augmentated image. 
...
* Frozen in Time: A Joint Video and Image Encoder for End-to-End Retrieval `UK` `Oxford`
	> Motivation: Train a CLIP-like model for video retrieval _efficiently_ -- on small datasets and preferably on image datasets if possible.
	
	> Method: The paper propose joint image-video training architecture. The architecture is mainly ViT-based that adds spatial and temporal positional embedding to every patch. After patch embedding, the paper utilize space-time Transformer to reduce the quadratic complexity of naive bi-directional embedding. Due to the architecture simplicity, it is trained using both image and video datasets. This increases the efficiency during training and versatility of the developed models in terms of both image and video support. To avoid the expensive computational cost of training on large scale noisy video datasets (e.g., HowTo100M), the paper introduce a new dataset WebVid-2M, with high quality labels and 20x smaller than HowTo100M. 
...
* MDETR - Modulated Detection for End-to-End Multi-Modal Understanding `FAIR` `US` `NYU`
	> This is a dense paper. 
	> Motivation: Object detection models are trained on a fixed vocabulary. Accordingly, they struggle with long-tail visual concepts. This paper proposes MDETR, a modulated (text-conditioned) object detection model trained on text-image pairs. For instance, the paper uses RefCOCO and Flickr30k datasets which contain phrases (single and plural) with human bounding box annotations. 
	> Method: MDTER is similar to DETR in term of both having a vision encoder followed by an _encoder-decoder_ transformer. Yet, MDTER uses a text-encoder besides the vision-encoder. MDTER feeds both the text and vision encoders' outputs about to the _encoder-decoder_ transformer. The multi-modal (joint) text-image encoder enables training on free text (text-image) pairs instead of a fixed vocabulary. In terms of loss function, like DETR, MDTER uses localization loss (L1 + GIOU). Yet, MDTER introduces two additional loss terms: (1) soft token prediction, (2) contrastive alignment. Both loss terms align the output of the decoder (predicted bbox) with the output of the encoder (text/token embedding). The following figure highlights the difference between the two additional loss terms.
...
* Unsupervised Multi-Target Domain Adaptation Through Knowledge Distillation `CA` `SnT`
	> The paper proposed a domain adaptation for  classification problems. The paper assumes multiple target domains, and the goal is to learn a single student that generalizes across all domains instead of a student per domain. The paper shows that it is better to learn a teacher per domain instead of a single teacher across all domains. Then, the paper shows that it is better to distill knowledge from each teacher -- to the student -- independently, instead of averaging across teachers. So the paper iteratively distills target domain knowledge from multiple teachers to a common student.
...
* Robust and Generalizable Visual Representation Learning via Random Convolutions
	> The paper proposes an augmentation technique, randconv, to mitigate CNN bias to texture and add inductive bias towards global shape information. RandConv applys a random conv kernel -- with random size k -- on the input image before feeding into the architecture. RandConv is supposed to increase the model robustness and help a model trained on single domain generalize across other domains.
	
* Tent: Fully Test-time Adaptation by Entropy Minimization `Adobe` `UC Berkely` `US`
	> The paper proposed fully test-time adaptation for tackling domain-gap between training and testing. The proposed approach, Tent, requires neither target label nor source data. So, Tent requires access to a trained model (weights) and target data (inputs without labels). Tent optimizes (fine-tunes) the model on the target data with the objective to minimize prediction entropy -- make the model more confident in its predictions. 